...
const R_LABELLED_VARIABLE = /(^|[^\\])\$(?:\{([a-zA-Z_][a-zA-Z0-9_]*)(?:\[([0-9]+)\])?\}|([a-zA-Z_][a-zA-Z0-9_]*))/g;
...
async function load(p_makefile, g_args) {
	let z_makefile;
	try {
		z_makefile = require(p_makefile);
	}
	catch(e_require) {
	...
	if('function' === typeof z_makefile) {
		k_makefile = new makefile(z_makefile(new helper_makefile()));
	}
	// descriptor is static
	else {
		k_makefile = new makefile(z_makefile);
	}
	...
	let a_makes = await k_makefile.make(a_targets, hm_runners);
	...
	if(g_args.watch) {
		for(let g_make of a_makes) {
			for(let s_file in g_make.files) {
				...
				fs.watchFile(s_file, () => {
					console.log('updated '+s_file);
					k_makefile.make(a_w_targets, new Map());
				});
			}
		}
...
class bash {
	...
	// constructor() {
	// 	this.proc = cp.spawn('/bin/bash');
	// }

	// async exec(s_cmds, h_variables) {
	// 	// exec
	// 	this.proc.stdin.write(`${s_prepare}${s_cmds}`);
	// }

	// kill(s_signal='SIGKILL') {
	// 	this.proc.kill(s_signal);
	// }
}
...
class makefile {
	constructor(h_makefile) {
		...
		// normalize makefile
		for(let s_key in h_makefile) {
			let z_value = h_makefile[s_key];
	...
	async run(s_target, g_scope, s_tag, g_state) {
		...
		if(hm_encountered.has(s_target)) {
			...
			if(!g_recipe.concurrency) {
				// warn
				console.warn(`target '${s_target}' was already run in another task stack:`
		...
		let dp_runner = new Promise(async(fk_run) => {
			...
			// only if target is newer than all dependencies
			if(g_context.mtime && a_deps.every(a => a.every(g => g.mtimes.every(xt_mtime => g_context.mtime >= xt_mtime)))) {
				// skip build
				debug.warn(`nothing to make for ${s_target}`.yellow);
			}
			// otherwise, build
			else {
			// if(!g_context.mtime || a_deps.some(a_mtimes => a_mtimes.some(xt_mtime => xt_mtime >= g_context.mtime))) {
			// 	debugger;
	...
	eval(s_str, g_context) {
		...
		// 	.replace(R_LABELLED_VARIABLE, (s_match, s_preceed, s_boundaried_label, s_index, s_plain_label) => {
		// 		let s_value = '';

		// 		// boundaried label
		// 		if(s_boundaried_label) {
		// 			// no such variable
		// 			if(!(s_boundaried_label in h_variables)) {
		// 				throw new Error(`reference to undefined variable: '${s_boundaried_label}'`);
		// 			}

		// 			// fetch value
		// 			let z_value = h_variables[s_boundaried_label];

		// 			// array
		// 			if(Array.isArray(z_value)) {
		// 				// index
		// 				if(s_index) {
		// 					let z_item = z_value[s_index];

		// 					if(null !== z_item && 'undefined' !== typeof z_item) {
		// 						s_value = z_item;
		// 					}
		// 				}
		// 				// no index
		// 				else {
		// 					s_value = z_value.join(' ');
		// 				}
		// 			}
		// 			// string
		// 			else if('string' === typeof z_value) {
		...
		// 			// otherwise it becomes empty string
		// 		}
		// 	});
	}
...
if(module === require.main) {
	let h_cli = require('commander')
		...
		.option('-f, --file', 'use specified makefile')
		.arguments('[targets...]')
		.parse(process.argv);
	...
	let p_makefile = path.join(process.cwd(), s_mk_file);

	load(p_makefile, g_args);
}
